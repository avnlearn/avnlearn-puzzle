...
class RandUnit:
            ...
            case ast.Call():
                ...
                match func_name:
                    ...
                    case "integer" if len(node.args) == 1:
                        number_args.update({"name": "Int", "unit": node.args[0]})
                    case "Fraction" | "FractionSlash" if len(node.args) >= 2:
                        p = node.args[0]
                        q = node.args[1]
                        p_name = self.NumberName(p)
                        q_name = self.NumberName(q)
                        number_args.update({"numerator": None, "denominator": None})
                        ...
                        if is_ast(self.denominator):
                            ...
                            number_args.update({"denominator": self.denominator})
                        ...
                        if unit_p == unit_q:
                            if (
                                ...
                                self.proper_fraction = random.choice([True, False])
                    ...
                    case "FixUnit" if node.args:
                        number_args.update({"name": "Fix+", "unit": node.args[0]})
                    case _:
    ...
    def Integer_Positive(self, unit, zero=None, **kwargs):
        unit = unit if isinstance(unit, dict) and unit else self.ast_expr_dict(unit)
        ...
        match unit.get("name") if isinstance(unit, dict) else unit:
            case "Z+" | "Z-" if isinstance(unit.get("unit"), (str, int)):
                ...
                if py_re.search(r"^(?P<Number>[1-9]+)(?P<Zero>0+)$", str(unit["unit"])):
                    ...
                    number = int(
                        "{}{}".format(
                            from_ast(self.RUnit(matches.group("Number"), "Z+", zero)),
                            matches.group("Zero"),
                        )
                    )
    ...
    def Decimal_Positive(self, unit, zero, **kwargs):
        ...
        match unit.get("name") if isinstance(unit, dict) else unit:
            case "D+" | "D-" if unit.get("Integer_Part") and unit.get("Decimal_Part"):
                Integer_Part = self.RUnit(unit["Integer_Part"], "Z+", True, **kwargs)
                Integer_Part = from_ast(Integer_Part)
                Decimal_Part = self.RUnit(unit["Decimal_Part"], "Z+", None, **kwargs)
                Decimal_Part = from_ast(Decimal_Part)
                Zero_Part = (
                    unit["Zero_Part"] if isinstance(unit["Zero_Part"], str) else ""
                )
                number = float(f"{Integer_Part}.{Zero_Part}{Decimal_Part}")
                return ast.Constant(value=number)
            case "D+" | "D-" if isinstance((node := unit.get("unit")), ast.Call):
                node.args = [self.RUnit(i, zero=zero, **kwargs) for i in node.args]
                return node
            case str() as name:
                self.name = name
                return self.RUnit(unit, name, zero=zero, **kwargs)
    ...
    def Irrational_Positive(self, unit, zero, **kwargs):
        ...
        match unit.get("name") if isinstance(unit, dict) else unit:
            ...
            case "T+" | "T-":
                ...
                match node:
                    case ast.Call():
                        node.args[0] = self.RUnit(node.args[0], zero=zero, **kwargs)
                        type_func = None
                        match node.func:
                            case ast.Name():
                                ...
                                case "cbrt":
                                    match name:
                                        case "Z+" | "D+" | "Z-" | "D-" | "Q+" | "Q-":
                                            args = self.__class__(
                                                number=str(
                                                    Eval(node.args[0], sympy=True) ** 3
                                                )
                                            ).__str__()
                                            node.args[0] = to_ast(args)
                                        case _:
                                            node.args[0] = to_ast(
                                                f"({from_ast(node.args[0])})**3"
                                            )
                                case "root":
                                    name_exponent = self.NumberName(node.args[1])
                                    num_exponent = Eval(node.args[1], sympy=True)
                                    odd_number = True
                                    if num_exponent % 2 == 0:
                                        odd_number = False
                                    if name_exponent in ("Z+", "D+", "Z-", "D-"):
                                        match name:
                                            ...
                                            case _:
                                                node.args[0] = to_ast(
                                                    f"({from_ast(node.args[0])})**{num_exponent}"
                                                )
                    case ast.Attribute() if isinstance(node.value, ast.Name):
                        return node
                    case ast.Attribute() if isinstance(node.value, ast.Attribute):
                        node.value = self.RUnit(node.value, zero=zero, **kwargs)
                ...
                match node:
                    ...
                    case ast.Call() if len(node.args) > 0:
                        if self.base_change != False:
                            node.args[0] = self.RUnit(node.args[0], zero=zero, **kwargs)
                        if self.power_change != False:
                            node.args[1] = self.RUnit(node.args[1], zero=zero, **kwargs)
                    case _:
                        return ast.Constant(value=None)
                return node
            case str() as name:
                ...
                match node:
                    case ast.Call():
                        ...
                        for i in range(len(node.args)):
                            node.args[i] = self.RUnit(node.args[i], zero=zero, **kwargs)
    ...
    def Set_Positive(self, unit, zero, **kwargs):
        ...
        match unit.get("name") if isinstance(unit, dict) else unit:
            ...
            case "Coordinate+" | "Coordinate-" | "List+" | "List-" if (
                ...
                node.elts = [self.RUnit(i, zero=zero, **kwargs) for i in node.elts]
                return node
            case str() as name:
                self.name = name
                return self.RUnit(unit, name, zero=zero, **kwargs)
                ...
                if any(unit.get(i) == "-" for i in ["neg_p", "neg_q"]):
                    ...
                    match random.randrange(0, 3):
                        case 0 | 1 as index_neg:
                            unit.update({"neg_p" if index_neg == 0 else "neg_q": "-"})
                        case 2:
